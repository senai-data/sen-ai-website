...
class ScanRedditThread(Base):
    ...
    per (scan, Reddit URL) where the URL was cited by at least one LLM during
    the scan. Bundles brand-mention classification + Haiku sentiment + a
    composite leverage_score that drives UI sort order. PARITÉ obligatoire
...
class ScanCompetitorPage(Base):
    ...
    JSON-LD schemas (S6) and Babbar backlinks (MR.1). PARITÉ obligatoire
    avec worker/models.py.
    """
...
class ScanSentimentJudgement(Base):
    ...
    PARITÉ obligatoire avec worker/models.py.
    """
