...
async def get_competitor_reverse(scan_id: str, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for bucket in by_brand.values():
        ...
        # Sprint 7.2 - LLM recommendation card. Generated by audit_competitor_pages
        # at the end of the audit and persisted into scans.summary.competitor_recommendations
        # keyed by brand_id. Absent for legacy scans / brands with <3 audited pages.
        recos_payload = ((scan.summary or {}).get("competitor_recommendations") or {}).get(bucket["brand_id"])
        ...
        competitors.append({
            ...
            "llm_recommendations": recos_payload,
        })
