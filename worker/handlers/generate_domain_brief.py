...
def execute(job_payload: dict, scan_id: str, db: Session) -> dict:
    ...
    # BYOK : resolve the 3 tiers ONCE at the top (raises ByokKeyInvalid /
    # ByokCapExceeded before any spend when an org key is configured but
    # unusable - never a silent platform fallback).
    from services.byok import resolve_anthropic_key, resolve_openai_key, resolve_org_key
    openai_key, openai_src = resolve_openai_key(db, scan.client_id)
    gemini_org = resolve_org_key(db, scan.client_id, "gemini")
    anthropic_key, anthropic_src = resolve_anthropic_key(db, scan.client_id)
    ...
    try:
        parsed, raw, usage = _try_openai(domain, openai_key, primary_model)
    ...
    if brief is None and anthropic_key:
        ...
        try:
            parsed, raw, usage = _try_claude(domain, anthropic_key, claude_model)
