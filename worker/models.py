...
class ScanRedditThread(Base):
    ...
    # detect head-to-head you_lost. PARITÉ avec api/models.py.
...
class ScanPROutreach(Base):
    ...
    during this scan for a competitor or the focus brand. See
    worker/handlers/build_pr_outreach.py. PARITÉ obligatoire avec
    api/models.py.
    """
...
class ScanInternalLink(Base):
    ...
    LLM during the scan. Stores the parsed outbound internal-link graph
    + per-page anchor quality issues. PARITÉ obligatoire avec
    api/models.py.
    """
...
class ScanCrisisSignal(Base):
    ...
    multilingual heuristics. PARITÉ obligatoire avec api/models.py.
    """
...
class LlmUsageLog(Base):
    ...
    # priced at the cached rate. PARITÉ avec api/models.py.
